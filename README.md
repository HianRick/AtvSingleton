...
Que problemas poderiam acontecer se cada órgão tivesse sua própria instância de central (por exemplo, perder mensagens, 
históricos diferentes, falta de coordenação)?
R: Complementando a resposta anterior, atraso no atendimento e péssima organização.

Sua implementação é thread-safe? Se não, o que poderia ser feito para torná-la segura em um cenário com múltiplas threads 
(por exemplo, vários módulos disparando alertas ao mesmo tempo)?
